# NOTE: This is the script for the TAS1R1 PAML sequence run for the null 
...
dir_prefix <- "null_TAS1R3_" # Define the directory name prefix
files_to_copy <- c("2NG.dN", "2NG.dS", "2NG.t", "4fold.nuc", "Inf", "rst", 
                   "rst1", "rub", "test_output", 
                   "TAS1R3_revised_tree_following_error_message.txt")
new_dir <- "~/Documents/paml-4.10.6/bin/"

# ITERATIVELY CHANGE THE TREE FILE  
for (i in 1:20) {
  new_filename <- paste0("TAS1R3_revised_tree_following_error_message", i, ".txt") # Construct the new file name using paste()
  # tree <- readLines(new_filename) # Use the new file name in your file processing code
  file.rename(new_filename, "TAS1R3_revised_tree_following_error_message.txt")
  system("./codeml") # Execute command using system()
  dir_name <- paste0(dir_prefix, i) 
